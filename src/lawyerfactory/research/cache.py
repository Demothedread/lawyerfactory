...
class LegalResearchCacheManager:
    ...
    def __init__(
        self, enhanced_kg: EnhancedKnowledgeGraph, max_cache_size_mb: int = 500
    ):
        self.kg = enhanced_kg
    ...
    async def get_cached_research(
        self, cache_key: str, jurisdiction: str
    ) -> Optional[Dict[str, Any]]:
        """Get cached research results"""
        return await self._get_cache_entry(
            "legal_research_cache", cache_key, jurisdiction
        )
    ...
    async def get_cached_definition(
        self, legal_term: str, jurisdiction: str
    ) -> Optional[Dict[str, Any]]:
    ...
    async def get_cached_case_law(
        ...
        cached_result = await self._get_cache_entry(
            "case_law_cache", cache_key, jurisdiction
        )
        return cached_result.get("cases", []) if cached_result else None

    async def cache_case_law(
        self,
        cause_of_action: str,
        jurisdiction: str,
    ...
    async def _get_cache_entry(
        self, cache_type: str, cache_key: str, jurisdiction: str
    ) -> Optional[Dict[str, Any]]:
    ...
    def _update_hit_metrics(self, cache_type: str, response_time_ms: float):
        ...
        if metrics.average_response_time_ms == 0:
            ...
            metrics.average_response_time_ms = (
                metrics.average_response_time_ms * (metrics.total_hits - 1)
                + response_time_ms
            ) / metrics.total_hits
    ...
    async def _check_cache_capacity(self, cache_type: str, new_entry_size: int) -> bool:
        ...
        try:
            ...
            current_size_bytes = (
                current_size[0] if current_size and current_size[0] else 0
            )
    ...
    async def invalidate_cache(
        self, cache_type: str, pattern: str = None, jurisdiction: str = None
    ) -> int:
        ...
        while True:
            try:
                ...
                for cache_type in self.cache_config.keys():
                    ...
                    if expired_count > 0:
                        logger.info(
                            f"Cleaned {expired_count} expired entries from {cache_type}"
                        )
                ...
                for cache_type, config in self.cache_config.items():
                    ...
                    if (
                        ...
                        > self.max_cache_size_mb * config["cleanup_threshold"]
                    ):
        ...
        while True:
            try:
                ...
                for cache_type in self.cache_config.keys():
                    ...
                    self.performance_metrics[cache_type].cache_size_mb = size_info[
                        "size_mb"
                    ]
        ...
        try:
            stats = {
                "total_performance": {
                    ...
                    "total_hits": sum(
                        m.total_hits for m in self.performance_metrics.values()
                    ),
                    "total_misses": sum(
                        m.total_misses for m in self.performance_metrics.values()
                    ),
                    "overall_hit_rate": 0.0,
                    "total_evictions": sum(
                        m.eviction_count for m in self.performance_metrics.values()
                    ),
                },
                "by_cache_type": {},
            }
    ...
    async def optimize_cache_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            optimization_results["actions_taken"].append(
                "Performed automatic cache optimization"
            )
    ...
    async def _auto_optimize_cache(self):
        ...
        try:
            ...
            for cache_type in self.cache_config.keys():
                ...
                deleted_count = self.kg._execute(
                    f"""
                    DELETE FROM {cache_type} 
                ...
                if deleted_count > 0:
                    logger.info(
                        f"Auto-optimized {cache_type}: removed {deleted_count} old low-relevance entries"
                    )
    ...
    async def preload_common_definitions(
        self, jurisdiction: str, legal_terms: List[str]
    ):
