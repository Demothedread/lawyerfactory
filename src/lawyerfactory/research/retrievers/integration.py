...
# Description: Legal Research API Integration System for Claims Matrix Phase 3.2 Provides comprehensive legal research capabilities with CourtListener, Google Scholar, and OpenAlex integration
# Relationships:
#   - Entity Type: Module
...
class LegalResearchAPIIntegration:
    ...
    def __init__(
        ...
        # Cache management
        self.cache_stats = {"hits": 0, "misses": 0, "invalidations": 0, "size_mb": 0.0}
    ...
    async def submit_research_request(self, request: LegalResearchRequest) -> str:
        ...
        logger.info(
            f"Submitted research request {request.request_id} with priority {request.priority.value}"
        )
        return request.request_id

    async def execute_research_request(
        self, request: LegalResearchRequest
    ) -> ResearchResult:
        ...
        try:
            ...
            scored_citations = await self._apply_relevance_scoring(
                all_citations, request
            )
            validated_citations = await self._validate_legal_authorities(
                scored_citations, request
            )

            # Generate legal analysis
            legal_principles = await self._extract_legal_principles(
                validated_citations, request
            )
            gaps_identified = await self._identify_research_gaps(
                validated_citations, request
            )
            ...
            logger.info(
                f"Completed research request {request.request_id} with {len(validated_citations)} citations"
            )
            return result

        except Exception as e:
            logger.exception(
                f"Failed to execute research request {request.request_id}: {e}"
            )
            raise

    async def _build_research_queries(
        self, request: LegalResearchRequest
    ) -> List[ResearchQuery]:
    ...
    async def _research_courtlistener(
        self, query: ResearchQuery, request: LegalResearchRequest
    ) -> List[LegalCitation]:
        ...
        try:
            ...
            results = await self.courtlistener_client.search_opinions(
            ...
            logger.info(
                f"CourtListener research returned {len(citations)} citations for {query.query_text}"
            )
    ...
    async def _research_google_scholar(
        self, query: ResearchQuery, request: LegalResearchRequest
    ) -> List[LegalCitation]:
        ...
        try:
            ...
            logger.info(
                f"Google Scholar research returned {len(citations)} citations for {query.query_text}"
            )
    ...
    async def _research_openalex(
        ...
        try:
            ...
            logger.info(
                f"OpenAlex research returned {len(citations)} citations for {query.query_text}"
            )
    ...
    async def _parse_courtlistener_result(
        self, result: Dict, request: LegalResearchRequest
        ...
        try:
            ...
            citation = LegalCitation(
                ...
                year=(
                    result.get("dateFiled", "")[:4] if result.get("dateFiled") else None
                ),
    ...
    def _is_jurisdiction_compatible(
        ...
        return self.jurisdiction_manager.is_jurisdiction_compatible(
            result_jurisdiction, request_jurisdiction
        )

    async def _apply_relevance_scoring(
        self, citations: List[LegalCitation], request: LegalResearchRequest
    ) -> List[LegalCitation]:
    ...
    async def _validate_legal_authorities(
        self, citations: List[LegalCitation], request: LegalResearchRequest
    ) -> List[LegalCitation]:
        ...
        for citation in citations:
            ...
            if validation.is_valid:
                ...
                logger.debug(
                    f"Validated citation: {citation.citation} (score: {validation.confidence_score:.2f})"
                )
            else:
                logger.debug(
                    f"Rejected citation: {citation.citation} - {validation.validation_notes}"
                )

        return validated_citations

    async def _validate_single_authority(
        self, citation: LegalCitation, request: LegalResearchRequest
    ) -> AuthorityValidation:
        """Validate a single legal authority"""
        try:
            # Check jurisdiction compatibility
            if not self._is_jurisdiction_compatible(
                citation.jurisdiction, request.jurisdiction
            ):
    ...
    def _calculate_authority_confidence(
        self, citation: LegalCitation, request: LegalResearchRequest
    ) -> float:
    ...
    async def _extract_legal_principles(
        ...
        top_citations = sorted(
            citations, key=lambda c: (c.authority_level, -c.relevance_score)
        )[:10]

        for citation in top_citations:
            if citation.excerpt:
                # Extract legal principles using keyword analysis
                excerpt_principles = self._analyze_text_for_principles(
                    citation.excerpt, request.cause_of_action
                )
                principles.extend(excerpt_principles)

        # Deduplicate and return most relevant principles
        unique_principles = list(
            dict.fromkeys(principles)
        )  # Preserve order while deduplicating
        return unique_principles[:15]  # Limit to most important principles

    def _analyze_text_for_principles(
        self, text: str, cause_of_action: str
    ) -> List[str]:
    ...
    async def _generate_research_recommendations(
        ...
        if any("Limited authority for element" in gap for gap in gaps):
            recommendations.append(
                "Conduct additional element-specific research using narrower search terms"
            )

        if any("No direct authority from" in gap for gap in gaps):
            recommendations.append(
                f"Search for {request.jurisdiction}-specific statutes and regulations"
            )

        if any("Limited recent authority" in gap for gap in gaps):
            recommendations.append(
                "Focus on cases from the last 3-5 years for current trends"
            )

        if any("Limited high-level authority" in gap for gap in gaps):
            recommendations.append(
                "Research Supreme Court and Appellate court decisions for binding precedent"
            )

        # General recommendations
        if len(citations) < 10:
            recommendations.append(
                "Expand search terms to include synonyms and related legal concepts"
            )

        if len([c for c in citations if c.citation_type == "statute"]) < 2:
            recommendations.append(
                "Research relevant statutes and regulatory authority"
            )
    ...
    async def _calculate_overall_confidence(
        ...
        avg_authority = (
            sum(authority_scores) / len(authority_scores) if authority_scores else 5
        )
        ...
        coverage_score = (
            len(covered_elements) / len(request.legal_elements)
            if request.legal_elements
            else 1.0
        )
        ...
        current_year = datetime.now().year
        recent_citations = [
            c for c in citations if c.year and c.year >= current_year - 5
        ]
        recency_score = min(
            len(recent_citations) / 5, 1.0
        )  # Normalize to max 5 recent citations
    ...
    async def _check_research_cache(
        ...
        try:
            ...
            """,
                (request.jurisdiction, cache_key),
            ).fetchone()
    ...
    def _generate_cache_key(self, request: LegalResearchRequest) -> str:
        ...
        key_components = [
            ...
            "|".join(
                sorted(request.fact_context[:5])
            ),  # Limit fact context for key generation
        ]
        key_string = "||".join(key_components)
        return hashlib.md5(key_string.encode()).hexdigest()

    async def _cache_research_result(
        self, request: LegalResearchRequest, result: ResearchResult
    ):
    ...
    async def _store_research_in_kg(
        self, request: LegalResearchRequest, result: ResearchResult
    ):
        ...
        try:
            ...
            logger.info(
                f"Stored {len(result.citations[:10])} citations in knowledge graph"
            )
                ...
                if pending_requests:
                    ...
                    for request in pending_requests:
                        ...
                        try:
                            ...
                            logger.error(
                                f"Background research failed for {request.request_id}: {e}"
                            )
    ...
    async def get_research_status(self, request_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                + timedelta(minutes=queue_size * 5),
            }
