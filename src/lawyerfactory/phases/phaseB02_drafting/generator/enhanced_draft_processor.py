...
class EnhancedDraftProcessor:
    ...
    def __init__(self, knowledge_graph_path: str = "knowledge_graphs/main.db"):
        ...
        if UNIFIED_STORAGE_AVAILABLE:
            try:
                ...
                logger.info("Unified storage initialized for enhanced draft processor")
    ...
    def process_fact_statement_drafts(
        ...
        logger.info(
            f"Fact statement processing complete: {result['total_entities']} entities, {result['total_relationships']} relationships, {result['conflicts_detected']} conflicts"
        )

        # Store processing result through unified storage (don't block on this)
        if self.unified_storage:
            try:
                object_id = await self.store_draft_processing_result(
                    result, session_id, "fact_statement"
                )
                if object_id:
    ...
    def process_case_complaint_drafts(
        ...
        for i, draft in enumerate(case_drafts):
            try:
                ...
                result = self.relationship_detector.process_draft_document(
                    content, "case_complaint", source_credibility=0.85
                )
        ...
        logger.info(
            f"Case/complaint processing complete: {result['total_entities']} entities, {result['legal_issues']['total_issues']} legal issues"
        )
        ...
        if self.unified_storage:
            try:
                object_id = await self.store_draft_processing_result(
                    result, session_id, "case_complaint"
                )
                if object_id:
                    result["storage_object_id"] = object_id
    ...
    def _aggregate_fact_statements(
    ...
    def _build_fact_timeline(
        ...
        for timeline_item in timeline:
            ...
            related_relationships = [
                ...
                if (
                    rel.get("from_entity") == timeline_item.get("entity_name")
                    or rel.get("to_entity") == timeline_item.get("entity_name")
                )
            ]
    ...
    def _analyze_legal_significance(
        ...
        for rel in relationships:
            ...
            if "critical" in significance or rel_type in [
                "causes",
                "plaintiff_defendant",
            ]:
                significance_categories["critical"].append(rel)
            elif "strengthens" in significance or rel_type in [
                "supports",
                "evidenced_by",
            ]:
    ...
    def _extract_legal_issues_structure(
        self, entities: List[Dict[str, Any]], relationships: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def _analyze_party_relationships(
        self, entities: List[Dict[str, Any]], relationships: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def _build_claims_hierarchy(
        ...
        for claim in claims:
            ...
            supporting_rels = [
                rel
                for rel in relationships
                if rel.get("to_entity") == claim_name
                and rel.get("type") in ["supports", "evidenced_by"]
            ]
    ...
    def _calculate_claim_strength(
        self, claim: Dict[str, Any], supporting_rels: List[Dict[str, Any]]
    ) -> float:
    ...
    def _identify_procedural_requirements(
    ...
    def _cross_validate_draft_information(
        self, fact_results: Dict[str, Any], case_results: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Cross-validate information between fact statements and case documents"""
        validation_results = {
    ...
    def _build_case_foundation(
        ...
        if fact_results:
            high_confidence_facts = [
                fact
                for category in fact_results.get("aggregated_facts", {})
                .get("categories", {})
                .values()
    ...
    def _assess_case_strength(self, foundation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "strength_rating": (
                ...
                else "moderate" if total_strength > 10 else "developing"
            ),
    ...
    def _generate_strength_recommendations(
        self, factual: int, legal: int, procedural: int
    ) -> List[str]:
        ...
        if factual < 5:
            recommendations.append(
                "Gather additional factual evidence to strengthen case foundation"
            )
        ...
        if factual > 10 and legal > 5:
            recommendations.append(
                "Case shows strong foundation - consider aggressive litigation strategy"
            )
    ...
    def _generate_processing_recommendations(
        ...
        if strength.get("factual_strength", 0) < 5:
            recommendations.append(
                "Consider gathering additional witness statements or documentation"
            )
    ...
    async def store_draft_processing_result(
        ...
        Args:
            processing_result: The processing result to store
            session_id: Session identifier
            result_type: Type of result (fact_statement, case_complaint, aggregated)

        Returns:
            ObjectID if stored successfully, None otherwise
        """
        if not self.unified_storage:
            logger.warning("Unified storage not available for storing draft processing results")
            return None
        ...
        try:
            ...
            if result_type == "fact_statement":
                metadata["draft_type"] = processing_result.get("draft_type", "unknown")
            ...
            logger.error(f"Failed to store draft processing result: {e}")
            return None

    async def retrieve_draft_processing_result(self, object_id: str) -> Optional[Dict[str, Any]]:
        """
        Retrieve draft processing result from unified storage

        Args:
            object_id: The ObjectID of the stored processing result

        Returns:
            Processing result data if found, None otherwise
        """
        if not self.unified_storage:
            logger.warning("Unified storage not available for retrieving draft processing results")
            return None
        ...
        try:
            ...
            if "evidence_data" in evidence_data:
                processing_json = evidence_data["evidence_data"]
                if isinstance(processing_json, str):
                    processing_data = json.loads(processing_json)
                else:
                    processing_data = processing_json

                return processing_data.get("processing_result")

            logger.warning("No evidence data found in retrieval result")
            return None

        except Exception as e:
            logger.error(f"Failed to retrieve draft processing result {object_id}: {e}")
            return None
    ...
    async def search_draft_processing_results(
        ...
        """
        if not self.unified_storage:
            logger.warning("Unified storage not available for searching draft processing results")
            return []
        ...
        try:
            ...
            for result in search_results:
                # Retrieve full processing result
                processing_result = await self.retrieve_draft_processing_result(
            ...
            logger.error(f"Failed to search draft processing results: {e}")
            return []
