...
class EnhancedPromptProcessor:
    ...
    async def process_case_prompt(
        ...
        try:
            ...
            analysis_result = await self.prompt_service.analyze_prompt(
                case_prompt, document_type_hint
            )
            ...
            enhanced_result = {
                **analysis_result,
                ...
                "case_data": case_data,
                "suggested_workflow_phases": self._suggest_workflow_phases(
                    analysis_result
                ),
    ...
    def _extract_case_data(self, analysis_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "extracted_entities": {
                "parties": [kw["text"] for kw in keywords if kw["type"] == "parties"],
                "legal_issues": [
                    kw["text"] for kw in keywords if kw["type"] == "legal_issue"
                ],
                "facts": [kw["text"] for kw in keywords if kw["type"] == "facts"],
                "relief_sought": [
                    kw["text"] for kw in keywords if kw["type"] == "relief_sought"
                ],
            },
            "confidence_metadata": {
                ...
                "requires_validation": analysis_result.get("confidence_score", 0.0)
                < 0.7,
            },
        }
    ...
    async def update_analysis_with_feedback(
        self, session_id: str, human_feedback: Dict[str, Any]
    ) -> Dict[str, Any]:
            ...
            for correction in human_feedback["keyword_corrections"]:
                ...
                for keyword in analysis["keywords"]:
                    if keyword["text"] == correction.get("original_text"):
                        keyword["text"] = correction.get(
                            "corrected_text", keyword["text"]
                        )
                        keyword["confidence"] = correction.get(
...
def create_prompt_processor(
    maestro: EnhancedMaestro, llm_service=None
