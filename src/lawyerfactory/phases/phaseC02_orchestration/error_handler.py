...
class WorkflowErrorHandler:
    ...
    async def handle_task_error(
        self, task: WorkflowTask, error: Exception, workflow_state: WorkflowState
    ):
    ...
    async def handle_agent_error(
        self, agent_type: str, error: Exception, workflow_state: WorkflowState
    ):
        """Handle errors related to agent execution"""
        error_info = self._create_error_info(
            error_type=ErrorType.AGENT_ERROR,
            error=error,
            context={"agent_type": agent_type, "session_id": workflow_state.session_id},
        )
    ...
    async def handle_workflow_error(
        self, workflow_state: WorkflowState, error: Exception
    ):
        ...
        if recovery_action == "pause":
            ...
            logger.info(
                f"Workflow {workflow_state.session_id} paused for manual intervention"
            )
    ...
    async def _should_retry_task(
        self, task: WorkflowTask, error_info: Dict[str, Any]
    ) -> bool:
    ...
    async def _schedule_task_retry(
        ...
        logger.info(
            f"Scheduling retry for task {task.id} in {delay} seconds (attempt {task.retry_count})"
        )
    ...
    async def _handle_permanent_task_failure(
        self,
        task: WorkflowTask,
        ...
        logger.warning(
            f"Task {task.id} permanently failed after {task.retry_count} retries"
        )
        ...
        dependent_tasks = [
            t for t in workflow_state.tasks.values() if task.id in t.depends_on
        ]

        if dependent_tasks:
            logger.warning(
                f"Task failure affects {len(dependent_tasks)} dependent tasks"
            )
    ...
    async def _schedule_workflow_retry(
        self, workflow_state: WorkflowState, error_info: Dict[str, Any]
    ):
    ...
    def _determine_recovery_action(
        self, error_info: Dict[str, Any], workflow_state: WorkflowState
    ) -> str:
    ...
    def _create_error_info(
        self, error_type: ErrorType, error: Exception, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "severity": self._assess_error_severity(
                {"error_type": error_type, "error_message": str(error)}
            ).value,
        }
    ...
    def _get_recent_errors(self, session_id: str, hours: int = 24) -> list:
        ...
        for error in self.error_history:
            ...
            if (
                ...
                and error["context"].get("session_id") == session_id
            ):
    ...
    def get_error_statistics(self, session_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if session_id:
            errors = [
                ...
                if e["context"].get("session_id") == session_id
            ]
        else:
