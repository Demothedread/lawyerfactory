...
from .enhanced_document_categorizer import (
    DocumentMetadata, DocumentType, DocumentAuthority,
    DocumentCluster
)
...
class VectorClusterManager:
    ...
    def _initialize_standard_clusters(self):
        ...
        for cluster_id in standard_clusters:
            ...
            self.cluster_metadata[cluster_id] = DocumentCluster(
                ...
                cluster_type=cluster_id.replace('_', ' ').title()
            )
    ...
    def create_defendant_cluster(self, defendant_name: str) -> str:
        ...
        if cluster_id not in self.clusters:
            ...
            self.cluster_metadata[cluster_id] = DocumentCluster(
                ...
                typical_similarity_threshold=self.validation_thresholds[DocumentType.PLAINTIFF_COMPLAINT]
            )

        return cluster_id

    async def add_document(self, document: DocumentMetadata, text_content: str,
        ...
        try:
            ...
            if not vector:
                logger.error(f"Failed to generate embedding for document {document.document_id}")
                return False
            ...
            if target_cluster not in self.clusters:
                if document.defendant_name:
                    target_cluster = self.create_defendant_cluster(document.defendant_name)
                else:
            ...
            logger.info(f"Added document {document.document_id} to cluster {target_cluster}")
            return True
    ...
    def _determine_cluster(self, document: DocumentMetadata) -> str:
        ...
        if (document.document_type == DocumentType.PLAINTIFF_COMPLAINT
    ...
    async def find_similar_documents(self, query_document: DocumentMetadata,
        ...
        """
        try:
            if not query_document.cluster_id or query_document.cluster_id not in self.clusters:
                return []
    ...
    async def validate_draft_against_cluster(self, draft_text: str,
                                           defendant_name: str,
        ...
        try:
            ...
            if cluster_id not in self.clusters:
                return {
                    ...
                    'recommendations': ['Upload more complaints against this defendant first']
                }
    ...
    async def get_cluster_analysis(self, cluster_id: str) -> Optional[ClusterAnalysis]:
        ...
        try:
            ...
            for doc in docs:
                doc_types[doc.metadata.document_type] = doc_types.get(doc.metadata.document_type, 0) + 1
                authority_levels[doc.metadata.authority_level] = authority_levels.get(doc.metadata.authority_level, 0) + 1
    ...
    def _calculate_cluster_quality(self, docs: List[VectorDocument]) -> float:
        ...
        quality_factors.append(1.0 - abs(0.3 - type_diversity))  # Optimal diversity around 30%

        # Authority level balance
        authority_levels = [doc.metadata.authority_level for doc in docs]
        binding_count = sum(1 for auth in authority_levels if auth == DocumentAuthority.BINDING_PRECEDENT)
        authority_balance = binding_count / len(docs)
        quality_factors.append(min(1.0, authority_balance + 0.5))  # Prefer some binding precedent
    ...
    async def _save_document(self, vector_doc: VectorDocument, cluster_id: str):
        ...
        try:
            ...
            logger.error(f"Failed to save document {vector_doc.metadata.document_id}: {e}")
    ...
    async def load_cluster(self, cluster_id: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Loaded {len(self.clusters[cluster_id])} documents for cluster {cluster_id}")
            return True
