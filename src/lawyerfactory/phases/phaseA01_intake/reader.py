...
# Static-only import for type checking, runtime import with fallback below
if TYPE_CHECKING:
    from ...compose.maestro.registry import AgentConfig, AgentInterface 

# Robust runtime import: prefer real AgentConfig/AgentInterface but provide lightweight fallbacks
try:
    from ...compose.maestro.registry import AgentConfig
    from ...compose.maestro.registry import AgentInterface as _AgentInterface  # type: ignore
except Exception:
    # Minimal runtime fallbacks so this module can be imported even if agent_registry is unavailable
    class AgentConfig:
        """Fallback AgentConfig (runtime only). Real project type is used for static checking."""
    ...
    class _AgentInterface:
        """Lightweight runtime fallback for AgentInterface."""

        def __init__(self, config: AgentConfig | None = None):
            self.config = config
            # basic runtime attributes expected by the reader
            self.is_busy = False
...
class ReaderBot(Bot, _AgentInterface):
    """Document reader bot for intake and text extraction using assessor module"""

    def __init__(self, config: AgentConfig):
        # Initialize Bot interface
        Bot.__init__(self)
        # Initialize AgentInterface (runtime alias)
        _AgentInterface.__init__(self, config)
    ...
    async def process_task(self, task: WorkflowTask) -> dict:
        ...
        try:
            ...
            if ASSESSOR_AVAILABLE:
                ...
                summary = summarize(description, max_sentences=1)
                return {
                    "status": "completed",
                ...
                return {
                    "status": "completed",
                    "message": "Document processed successfully (assessor unavailable)",
