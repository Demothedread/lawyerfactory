...
# Initialize qdrant_client only if available and configuration present.
# Defer logging of init issues until logger is configured later in the module.
qdrant_client = None
if QDRANT_AVAILABLE and os.environ.get("QDRANT_API_KEY"):
    try:
        qdrant_client = QdrantClient(
            url=os.environ.get(
                "QDRANT_URL",
                "https://4ce95fca-f9f6-4152-ad39-b059a7f4dfdd.us-west-1-0.aws.cloud.qdrant.io:6333",
            ),
            api_key=os.environ.get("QDRANT_API_KEY"),
        )
    except Exception as exc:
        # initialization failed; will fall back to None.
        # logger is configured now so use it
        logger.warning(f"Qdrant client initialization failed: {exc}")
        qdrant_client = None

# Test Qdrant connection (optional - only if debugging)
if os.environ.get("DEBUG_QDRANT") == "true" and qdrant_client is not None:
    try:
...
# aiohttp server fallback — provide a minimal fallback object immediately so static analysis sees expected attributes.
try:
    ...
    class _RouterFallback:
        def __init__(self):
            self.routes = []

        def add_post(self, path, handler):
            # store route for inspection but do not attempt to run
    ...
    class _WebFallback:
        ...
        class Application:
            def __init__(self, *a, **k):
                # provide router to satisfy create_app usage
                self.router = _RouterFallback()
        ...
        class AppRunner:
            ...
            async def setup(self):
                # explicit runtime error — informs user to install aiohttp
                raise RuntimeError("aiohttp not installed; AppRunner.setup unavailable")
        ...
        class TCPSite:
            ...
            async def start(self):
                raise RuntimeError("aiohttp not installed; TCPSite.start unavailable")
...
async def check_unified_storage_health():
    ...
    def intake_document(*args, **kwargs):
        # logger is defined earlier so safe to call
...
async def _chunk_and_vectorize(
    text: str,
...
async def _analyze_with_openai(
    ...
    try:
        ...
        async def _determine_type_and_group(
        ...
        def _extract_tags(
            ...
            text_blob = " ".join(
                [" ".join(categories or []), title_text or "", assistant_text or ""]
            ).lower()
        ...
        if openai_client and OPENAI_SDK_AVAILABLE and OPENAI_API_KEY:
            ...
            publication_date = (
                ...
                or (parsed.get("publication_date") if parsed else None)
            ...
            type_val, group_val = await _determine_type_and_group(
            ...
            type_val, group_val = await _determine_type_and_group(
                None, categories, metadata.get("title") or ""
            )
...
async def _process_draft_document(
    content: str, metadata: Dict[str, Any], draft_type: str
) -> Dict[str, Any]:
...
async def _aggregate_draft_documents(
    draft_list: List[Dict[str, Any]], draft_type: str
) -> Dict[str, Any]:
...
async def _process_with_facts_matrix_adapter(
    ...
    try:
        ...
        if FactsMatrixAdapter.validate_facts_matrix(facts_matrix):
            ...
            logger.info(
                f"Generated facts matrix with {len(facts_matrix.get('undisputed_facts', []))} undisputed facts, "
...
async def _ingest_to_knowledge_graph(
    entities: List[Dict[str, Any]], metadata: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"KG ingestion successful: {entities_stored} entities from {metadata.get('draft_type', 'unknown')} draft"
            )
            return kg_result
            ...
            logger.info(
                f"Simulated KG ingestion: {len(entities)} entities from {metadata.get('draft_type', 'unknown')} draft"
            )
            return kg_result
    ...
    try:
        ...
        if "legal_entities" in draft_analysis:
            kg_result = await _ingest_to_knowledge_graph(
                draft_analysis.get("legal_entities", []), metadata
            )
