...
from lawyerfactory.document_generator.ai_document_generator import \
    AIDocumentGenerator
...
class AIDocumentAgent(AgentInterface):
    ...
    def __init__(self, config: AgentConfig):
        ...
        forms_directory = config.config.get('forms_directory', 'docs/Court_files') if config.config else 'docs/Court_files'
        output_directory = config.config.get('output_directory', 'output') if config.config else 'output'
        ...
        self.processing_timeout = config.config.get('processing_timeout', 300) if config.config else 300  # 5 minutes
    ...
    async def execute_task(self, task: WorkflowTask, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        logger.info(f"AI Document Agent executing task {task.id} in phase {task.phase.value}")
        
        try:
            # Extract case data from context
            case_data = self._extract_case_data(context)
            case_name = context.get('case_name', f"Case_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
            
            # Determine task type based on phase and description
            if task.phase == WorkflowPhase.DRAFTING:
                return await self._handle_drafting_task(task, case_data, case_name, context)
                ...
                return await self._handle_form_processing_task(case_data, case_name, context)
                ...
                return await self._handle_form_generation_task(case_data, case_name, context)
            else:
                # Default to full document generation
                return await self._handle_full_generation_task(case_data, case_name, context)
    ...
    async def _handle_drafting_task(self, task: WorkflowTask, case_data: Dict[str, Any], 
                                   case_name: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'case_classification': result.case_classification.primary_type.value if result.case_classification else None,
            'form_types': [mapping.form_name for mapping in result.form_mappings] if result.form_mappings else [],
            'output_files': [str(filling.output_path) for filling in result.filling_results if filling.success],
    ...
    async def _handle_classification_task(self, case_data: Dict[str, Any], 
        ...
        return {
            ...
            'classification': {
                ...
                'secondary_types': [(t[0].value, t[1]) for t in classification.secondary_types],
    ...
    async def _handle_form_processing_task(self, case_data: Dict[str, Any], 
                                         case_name: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'optional_forms': [form.form_name for form in form_selection.optional_forms],
    ...
    async def _handle_field_mapping_task(self, case_data: Dict[str, Any], 
                                       context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        mappings = self.ai_generator._map_fields(form_selection, case_data, classification)
        
        total_mappings = sum(len(mapping.mappings) for mapping in mappings)
        successful_mappings = sum(len([m for m in mapping.mappings if m.confidence > 0.5]) for mapping in mappings)
            ...
            'mapping_details': [
                {
                    ...
                    'mapped_fields': len([m for m in mapping.mappings if m.confidence > 0.3]),
    ...
    async def _handle_form_generation_task(self, case_data: Dict[str, Any], 
                                         case_name: str, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _handle_full_generation_task(self, case_data: Dict[str, Any], 
                                         case_name: str, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _run_ai_generation(self, case_data: Dict[str, Any], case_name: str, 
        ...
        return await loop.run_in_executor(
            None,
            self.ai_generator.generate_documents,
            case_data,
            case_name,
            options
        )
    ...
    async def initialize(self) -> None:
        ...
        if not self.ai_generator.forms_directory.exists():
            logger.warning(f"Forms directory not found: {self.ai_generator.forms_directory}")
