...
class FactChecker:
    ...
    async def verify_document_facts(
        ...
        for fact in extracted_facts:
            result = await self._verify_single_fact(
                fact, source_materials, verification_level
            )
        ...
        logger.info(
            f"Fact verification completed. {report.verified_facts}/{report.total_facts_checked} facts verified"
        )
        return report
    ...
    def _classify_fact_type(self, fact_text: str) -> str:
        ...
        if any(keyword in fact_lower for keyword in ["date", "on", "during", "at"]):
            ...
            keyword in fact_lower for keyword in ["$", "dollar", "amount", "cost"]
        ):
            return "financial"
        elif any(
            keyword in fact_lower for keyword in ["court", "judge", "case", "ruling"]
        ):
            return "legal"
        elif any(
            keyword in fact_lower for keyword in ["plaintiff", "defendant", "party"]
        ):
            return "party"
        else:
            return "general"
    ...
    async def _verify_single_fact(
        ...
        result = FactVerificationResult(
            ...
            reviewer_suggestions=self._generate_reviewer_suggestions(
    ...
    def _search_facts_matrix(
        self, fact: Dict[str, Any], facts_matrix: Dict[str, Any]
    ) -> List[str]:
            ...
            if category in facts_matrix:
                for fact_item in facts_matrix[category]:
                    if isinstance(fact_item, str):
                        if self._has_semantic_overlap(
                            fact_text_lower, fact_item.lower()
                        ):
                            evidence.append(f"Facts Matrix ({category}): {fact_item}")

        return evidence

    def _search_case_documents(
        self, fact: Dict[str, Any], case_documents: List[Dict[str, Any]]
    ) -> List[str]:
        ...
        for doc in case_documents:
            if "content" in doc:
                ...
                if self._has_semantic_overlap(fact_text_lower, doc_content_lower):
                    ...
                    evidence.append(
                        f"Document ({doc_name}): Content contains related information"
                    )
    ...
    def _generate_reviewer_suggestions(
        self, status: FactStatus, confidence: float
    ) -> Optional[str]:
    ...
    def _generate_verification_report(
        ...
        unverified_count = sum(
            1 for r in fact_results if r.status == FactStatus.UNVERIFIED
        )
        contradicted_count = sum(
            1 for r in fact_results if r.status == FactStatus.CONTRADICTED
        )
        needs_review_count = sum(
            1 for r in fact_results if r.status == FactStatus.NEEDS_REVIEW
        )

        if total_facts > 0:
            overall_confidence = (
                sum(r.confidence_score for r in fact_results) / total_facts
            )
        else:
        ...
        summary_lines = [
            ...
            (
                f"Overall accuracy: {(verified_count/total_facts)*100:.1f}%"
                if total_facts > 0
                else "No facts to verify."
            ),
        ]

        if contradicted_count > 0:
            summary_lines.append(
                f"⚠️ {contradicted_count} facts contradicted - immediate review required."
            )
        if needs_review_count > 0:
            summary_lines.append(
                f"📋 {needs_review_count} facts need additional review."
            )
...
async def verify_document_quick(
    document_content: str, source_materials: Dict[str, Any]
) -> VerificationReport:
    """Quick fact verification with basic level"""
    checker = FactChecker()
    return await checker.verify_document_facts(
        document_content, source_materials, VerificationLevel.BASIC
    )


async def verify_document_comprehensive(
    document_content: str, source_materials: Dict[str, Any], kg=None
) -> VerificationReport:
    """Comprehensive fact verification with knowledge graph support"""
    checker = FactChecker(knowledge_graph=kg)
    return await checker.verify_document_facts(
        document_content, source_materials, VerificationLevel.COMPREHENSIVE
    )
