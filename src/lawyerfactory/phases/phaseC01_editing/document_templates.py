...
{court_name}"""
    ...
    def format_document(
        self, document_type: DocumentType, content: Dict[str, Any]
    ) -> str:
    ...
    def _format_statement_of_facts(
        self, templates: Dict[str, str], content: Dict[str, Any]
    ) -> str:
        """Format Statement of Facts document"""
        document_parts = []

        # Header
        case_caption = self.get_case_caption_template(self.config.court_level).format(
            **content.get("case_info", {})
        )
        header = templates["header"].format(
            case_caption=case_caption,
            moving_party=content.get("moving_party", "Plaintiff"),
            document_purpose=content.get(
                "document_purpose", "in support of the pending motion"
            ),
            ...
            for i, fact in enumerate(section.get("facts", []), 1):
                if fact.get("is_disputed", False):
                    fact_para = templates["disputed_fact"].format(
                        ...
                        dispute_description=fact.get(
                            "dispute_description", "this allegation is accurate"
                        ),
                    )
                else:
        ...
        conclusion = templates["conclusion"].format(
            conclusion_statement=content.get(
                "conclusion_statement", "the facts support the relief requested"
            ),
            prayer_for_relief=content.get("prayer_for_relief", ""),
            signature_block=templates["signature_block"].format(
                **content.get("attorney_info", {})
            ),
        )
    ...
    def validate_citation_format(
        self, citation: str, citation_type: str
    ) -> Dict[str, Any]:
    ...
    def _get_citation_suggestions(self, citation: str, citation_type: str) -> List[str]:
        ...
        if citation_type == "case_citation":
            suggestions.append(
                "Ensure format: Case Name, Vol. Reporter Page (Court Year)"
            )
