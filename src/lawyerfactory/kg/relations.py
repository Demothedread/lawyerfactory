...
class LegalRelationshipDetector:
    ...
    def extract_legal_entities_from_text(
        self, text: str, draft_type: str = "unknown", source_credibility: float = 0.7
    ) -> List[LegalEntity]:
        ...
        for entity_type, patterns in self.legal_patterns.items():
            for pattern in patterns:
                ...
                for match in matches:
                    ...
                    if (
                        len(entity_text) < 2 or len(entity_text) > 200
                    ):  # Filter unreasonable matches
                        continue
    ...
    def extract_legal_relationships_from_text(
        self, text: str, entities: List[LegalEntity], source_credibility: float = 0.7
    ...
    def _calculate_entity_confidence(
        ...
        context = self._get_context_window(
            full_text, match.start(), match.end(), window_size=100
        )
        legal_terms = [
            "plaintiff",
            "defendant",
        ...
        legal_term_count = sum(
            1 for term in legal_terms if term.lower() in context.lower()
        )
    ...
    def _calculate_relationship_confidence(
        ...
        entity_confidence_avg = (
            from_entity.overall_confidence + to_entity.overall_confidence
        ) / 2
            ...
            if key not in seen_combinations:
                ...
                for existing in unique_entities:
                    if (
                        ...
                        and existing.name.lower().strip() == entity.name.lower().strip()
                    ):
    ...
    def process_draft_document(
        self, document_content: str, draft_type: str, source_credibility: float = 0.8
    ) -> Dict[str, Any]:
        """Process a complete draft document and extract entities and relationships"""
        logger.info(
            f"Processing {draft_type} draft document with {len(document_content)} characters"
        )
            ...
            "relationship_details": [
                {
                    ...
                    "from_entity": next(
                        (e.name for e in entities if e.id == rel.from_entity), "Unknown"
                    ),
                    "to_entity": next(
                        (e.name for e in entities if e.id == rel.to_entity), "Unknown"
                    ),
        ...
        logger.info(
            f"Completed processing: {result['entities_extracted']} entities, {result['relationships_extracted']} relationships"
        )
        return result
