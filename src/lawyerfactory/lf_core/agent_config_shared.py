...
class AgentSkill:
    ...
    def matches_requirement(
        self, required_skill: str, min_level: SkillLevel = SkillLevel.INTERMEDIATE
    ) -> bool:
        """Check if this skill matches a requirement"""
        return (
            self.name.lower() == required_skill.lower()
            and self.level.value >= min_level.value
...
class AgentConfiguration:
    ...
    def has_capability(self, capability_name: str, min_confidence: float = 0.5) -> bool:
        ...
        for cap in self.capabilities:
            if (
                cap.name.lower() == capability_name.lower()
                and cap.confidence >= min_confidence
            ):
                return True
        return False
...
class AgentConfigManager:
    ...
    def _create_general_agent_config(self) -> AgentConfiguration:
        ...
        return AgentConfiguration(
            ...
            preferred_phases=[
                WorkflowPhaseType.INTAKE,
                WorkflowPhaseType.ORCHESTRATION,
            ],
    ...
    def _create_legal_specialist_config(self) -> AgentConfiguration:
        ...
        return AgentConfiguration(
            ...
            preferred_phases=[
                WorkflowPhaseType.RESEARCH,
                WorkflowPhaseType.DRAFTING,
    ...
    def load_agent_config(self, config_file: Union[str, Path]) -> AgentConfiguration:
        ...
        preferred_phases = [
            WorkflowPhaseType(phase)
            for phase in config_data.get("preferred_phases", [])
        ]
    ...
    def find_best_agent(
        ...
        phase: WorkflowPhaseType,
        task_complexity: float = 0.5,
        agent_type_preference: Optional[AgentType] = None,
    ) -> Optional[str]:
        ...
        available_agents = [
            agent_id for agent_id in candidate_ids if self.agents[agent_id].availability
        ]
        ...
        for agent_id in available_agents:
            ...
            fitness_score = agent.calculate_fitness_score(
                required_skills, phase, task_complexity
            )
        ...
        if candidates:
            ...
            logger.info(
                f"Selected agent {best_agent_id} with fitness score {candidates[0][1]:.3f}"
            )
    ...
    def get_agent_workload(self, agent_id: str) -> int:
        ...
        return sum(
            1
            for assigned_agent in self.task_assignments.values()
            if assigned_agent == agent_id
        )

    def get_agents_by_skill(
        self, skill_name: str, min_level: SkillLevel = SkillLevel.INTERMEDIATE
    ) -> List[str]:
    ...
    def get_agent_pool_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "available_agents": sum(
                1 for agent in self.agents.values() if agent.availability
            ),
            "agent_types": {},
            "skills_coverage": {},
            ...
            if capability.name.lower() == capability_name.lower():
                ...
                if success:
                    capability.success_rate = (
                        capability.success_rate * (1 - alpha) + alpha
                    )
                else:
                    capability.success_rate = capability.success_rate * (1 - alpha)

                # Adjust confidence based on recent performance
                capability.confidence = min(
                    0.95, capability.confidence * capability.success_rate
                )

                logger.info(
                    f"Updated performance for agent {agent_id}, capability {capability_name}"
                )
                break
...
class AgentPoolManager:
    ...
    async def distribute_tasks(
        self, tasks: List[Dict[str, Any]], phase: WorkflowPhaseType
    ) -> Dict[str, List[str]]:
    ...
    async def complete_task(
        self, task_id: str, success: bool, execution_time: Optional[float] = None
    ):
    ...
    def get_load_balance_report(self) -> Dict[str, Any]:
        ...
        total_workload = sum(
            info["current_tasks"] for info in report["agent_workloads"].values()
        )
        total_capacity = sum(
            info["max_capacity"] for info in report["agent_workloads"].values()
        )
        ...
        report["utilization_stats"] = {
            ...
            "overall_utilization": (
                total_workload / total_capacity if total_capacity > 0 else 0
            ),
        }
