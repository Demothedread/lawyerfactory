"""
# Script Name: document_types_shared.py
# Description: Modular Document Type Framework for LawyerFactory Supports business proposals, white papers, and legal documents with extensible architecture. Implements 1-3-1 sandwich writing model with recursive document structuring.
# Relationships:
#   - Entity Type: Module
    ...
    def validate_structure(self, structure: DocumentStructure) -> List[str]:
        ...
        if structure.target_length:
            ...
            if actual_count < target * 0.8 or actual_count > target * 1.2:
                issues.append(
                    f"Word count {actual_count} outside target range {target}±20%"
                )
...
class SandwichWritingModel:
    ...
    def __init__(self):
        self.model_pattern = {"opening": 1, "body": 3, "closing": 1}
    ...
    def apply_model(self, structure: DocumentStructure) -> DocumentStructure:
        ...
        if len(sections) < 5:
            logger.warning(
                "Not enough sections for 1-3-1 model, keeping original structure"
            )
            return structure
        ...
        while len(body_sections) < 3:
            ...
            if largest.subsections:
                ...
                largest.subsections = largest.subsections[
                    : len(largest.subsections) // 2
                ]
    ...
    def _create_recursive_structure(
...
class DocumentTypeFramework:
    ...
    def register_template(
        self, document_type: DocumentType, template: DocumentTemplate
    ):
    ...
    def create_document_structure(
        ...
        apply_sandwich_model: bool = True,
    ) -> DocumentStructure:
    ...
    def export_structure(
        self, structure: DocumentStructure, format: str = "json"
    ) -> str:
