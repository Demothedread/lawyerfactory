...
class CloudStorageManager:
    ...
    def __init__(
        ...
        s3_service=None,
        local_temp_dir: str = "./uploads/tmp",
    ):
    ...
    async def store_evidence_with_cloud_backup(
        self,
        content: str,
        ...
        try:
            ...
            doc_id = await self.vector_store.ingest_evidence(
                content=content, metadata=metadata, store_type=store_type
            )
            ...
            cloud_info = None
            if self.s3_service:
                cloud_info = await self._upload_to_cloud(
                    local_path, storage_id, metadata
                )
    ...
    async def _create_local_backup(
        self, content: str, storage_id: str, metadata: Dict[str, Any]
    ) -> Optional[Path]:
    ...
    async def _upload_to_cloud(
        self, local_path: Path, storage_id: str, metadata: Dict[str, Any]
        ...
        try:
            ...
            cloud_result = await self._call_s3_upload_service(
                content=content, filename=local_path.name, metadata=upload_metadata
            )
    ...
    async def _get_from_vector_store_only(self, doc_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"Vector store retrieval failed: {e}"}

    async def _call_s3_upload_service(
        ...
        try:
            ...
            for mapping in self.storage_mappings.values():
                if mapping.get("local_path"):
                    ...
                    if (
                        local_path.exists()
                        and datetime.fromtimestamp(local_path.stat().st_mtime)
                        < cutoff_date
                    ):
                        local_path.unlink()
...
class IntegratedEvidenceIngestion:
    ...
    def __init__(
        ...
        self.vector_store = vector_store_manager or EnhancedVectorStoreManager()
        self.cloud_storage = cloud_storage_manager or CloudStorageManager(
            vector_store_manager
        )

    async def process_evidence_with_storage(
        self,
        content: str,
        metadata: Dict[str, Any],
        store_type: VectorStoreType = VectorStoreType.PRIMARY_EVIDENCE,
