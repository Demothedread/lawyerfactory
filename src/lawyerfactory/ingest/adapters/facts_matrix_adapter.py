...
class FactsMatrixAdapter:
    ...
    def transform_to_facts_matrix(raw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            facts_matrix["case_metadata"] = FactsMatrixAdapter._extract_case_metadata(
                raw_data
            )

            # Extract evidence references
            facts_matrix["evidence_references"] = (
                FactsMatrixAdapter._extract_evidence_references(raw_data)
            )
            ...
            facts_matrix["background_context"] = FactsMatrixAdapter._extract_facts(
                raw_data,
                ["background_context", "background", "context", "case_background"],
            )
            ...
            logger.info(
                f"Successfully transformed data: {len(facts_matrix['undisputed_facts'])} undisputed, "
        ...
        for key in possible_keys:
            if key in raw_data:
                ...
                if isinstance(facts, list):
                    return [
                        str(fact) for fact in facts if fact
