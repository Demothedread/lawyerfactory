...
class AIDocumentGenerator:
    ...
    def __init__(
        self,
        forms_directory: str = "docs/Court_files",
    ...
    def generate_documents(
        ...
        try:
            ...
            if classification.primary_type.value == "unknown":
                warnings.append(
                    "Case classification has low confidence - review may be needed"
                )
            ...
            if not form_selection.primary_forms:
                errors.append(
                    "No appropriate forms could be selected for this case type"
                )
                return self._create_error_result(
                    case_name, classification, errors, warnings, start_time
                )
            ...
            if not viable_mappings:
                errors.append(
                    "Insufficient field mappings - check case data completeness"
                )
                return self._create_error_result(
                    case_name, classification, errors, warnings, start_time
                )
            ...
            package_info = self._create_package(
                viable_mappings, case_name, case_data.get("case_number", "")
            )
            ...
            all_warnings.extend(
                classification.key_indicators[:2]
            )  # Add key indicators as info
            ...
            self.logger.info(
                f"AI document generation completed: {forms_generated} forms, "
            ...
            return self._create_error_result(
                case_name, None, errors, warnings, start_time
            )
    ...
    def _classify_case(self, case_data: Dict[str, Any]) -> CaseClassification:
        ...
        additional_context = {
            ...
            "has_injury": any(
                word in str(case_data).lower() for word in ["injury", "hurt", "damage"]
            ),
    ...
    def _select_forms(
        self, classification: CaseClassification, options: Dict[str, Any]
    ) -> FormSelectionResult:
    ...
    def _map_fields(
        ...
        primary_avg_confidence = (
            sum(m.mapping_confidence for m in mappings) / len(mappings)
            if mappings
            else 0
        )

        if (
            primary_avg_confidence > 0.5
        ):  # Only process optional forms if primary forms are well-mapped
            for form_rec in form_selection.optional_forms[
                :2
            ]:  # Limit to top 2 optional forms
                mapping = self.field_mapper.map_fields(
                    form_rec.form_name, case_data, classification
                )
                mappings.append(mapping)
    ...
    def _create_package(
        self, mappings: List[MappingResult], case_name: str, case_number: str
    ) -> Dict[str, Any]:
        """Create document package with all forms."""

        try:
            return self.form_filler.create_form_package(
                mappings, case_name, case_number
            )
        except Exception as e:
    ...
    def _create_error_result(
        self,
        case_name: str,
        classification: Optional[CaseClassification],
        errors: List[str],
        warnings: List[str],
        start_time: datetime,
    ) -> DocumentGenerationResult:
        ...
        return {
            ...
            "components": {
                "case_classifier": {
                    "status": "operational",
                    "supported_types": [
                        t.value for t in self.case_classifier.get_supported_case_types()
                    ],
                },
                "form_selector": {
                    "status": "operational",
    ...
    def analyze_case_requirements(self, case_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if form_selection.primary_forms:
            ...
            sample_mapping = self.field_mapper.map_fields(
                sample_form.form_name, case_data, classification
            )
        ...
        if classification.confidence < 0.7:
            recommendations.append(
                "Provide more specific case details to improve classification"
            )

        if sample_mapping and sample_mapping.mapping_confidence < 0.6:
            recommendations.append(
                "Add more structured case data for better form field mapping"
            )
        ...
        return {
            ...
            "data_completeness": {
                ...
                "completion_score": len(case_data)
                / (len(case_data) + len(missing_fields)),
            },
            "form_availability": {
            ...
            "mapping_preview": {
                "expected_confidence": (
                    sample_mapping.mapping_confidence if sample_mapping else 0.0
                ),
...
def test_ai_document_generator():
    ...
    try:
        ...
        print(
            f"Available Forms: {status['components']['form_selector']['available_forms']}"
        )
        ...
        print(
            f"  Primary Forms Available: {analysis['form_availability']['primary_forms']}"
        )
        ...
        print(
            "  Expected workflow: Classify → Select Forms → Map Fields → Fill PDFs → Package"
        )
