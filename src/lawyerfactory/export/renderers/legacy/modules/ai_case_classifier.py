...
class CaseClassifier:
    ...
    def _initialize_classification_patterns(self):
        ...
        self.case_patterns = {
            CaseType.BREACH_OF_CONTRACT: {
                ...
                "legal_elements": ["formation", "performance", "breach", "damages"],
            },
            CaseType.PERSONAL_INJURY: {
                ...
                "legal_elements": ["duty", "breach of duty", "causation", "damages"],
            },
            CaseType.NEGLIGENCE: {
                "keywords": [
                    "negligent",
            ...
            CaseType.PROPERTY_DAMAGE: {
                ...
                "legal_elements": ["ownership", "damage", "causation", "valuation"],
            },
            CaseType.EMPLOYMENT: {
                "keywords": [
                    "employment",
    ...
    def classify_case(
        ...
        combined_text = " ".join(
            [fact.get("text", "") for fact in case_facts if fact.get("text")]
        )
        ...
        if not sorted_scores or sorted_scores[0][1] == 0:
            ...
            secondary_types = [
                (case_type, score)
                for case_type, score in sorted_scores[1:]
                if score >= threshold
            ]
        ...
        reasoning = self._generate_reasoning(
            primary_type, confidence, key_indicators, damages_categories
        )
    ...
    def suggest_additional_facts(self, classification: CaseClassification) -> List[str]:
        ...
        if classification.confidence < 0.7:
            if classification.primary_type != CaseType.UNKNOWN:
                ...
                suggestions.append(
                    f"Consider gathering more facts about {case_type.value}"
                )

                if "legal_elements" in patterns:
                    missing_elements = [
                        elem
                        for elem in patterns["legal_elements"]
                        if not any(
                            elem in indicator
                    ...
                    if missing_elements:
                        suggestions.append(
                            f"Focus on evidence of: {', '.join(missing_elements)}"
                        )
...
def test_case_classifier():
    ...
    for test_case in test_cases:
        ...
        if classification.secondary_types:
            print(
                f"Secondary Types: {[(t.value, f'{s:.1%}') for t, s in classification.secondary_types]}"
            )
