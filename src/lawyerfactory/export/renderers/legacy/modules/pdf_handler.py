"""
# Script Name: pdf_handler.py
# Description: PDF Processing Module for LawyerFactory Document Generator  This module handles PDF form field extraction, manipulation, and filling for court document automation.
# Relationships:
#   - Entity Type: Module
...
class PDFHandler:
    ...
    def _extract_fields_with_pymupdf(self, pdf_path: Path) -> List[PDFFormField]:
        ...
        try:
            ...
            doc.close()
            self.logger.info(
                f"PyMuPDF extracted {len(fields)} potential fields from {pdf_path}"
            )
                ...
                for page_num, page in enumerate(reader.pages):
                    ...
                    if "/Annots" in page:
                        ...
                        writer.update_page_form_field_values(
                            writer.add_page(page), field_data
                        )
                    else:
    ...
    def validate_field_mapping(
        self, pdf_path: Union[str, Path], field_mapping: Dict[str, str]
    ) -> Dict[str, List[str]]:
        """
    ...
    def get_available_forms(self) -> Dict[str, Path]:
        ...
        if not self.forms_directory.exists():
            self.logger.warning(
                f"Forms directory does not exist: {self.forms_directory}"
            )
            return forms
    ...
    def analyze_form_structure(self, pdf_path: Union[str, Path]) -> Dict[str, Any]:
        ...
        for field in fields:
            ...
            analysis["field_types"][field_type] = (
                analysis["field_types"].get(field_type, 0) + 1
            )
