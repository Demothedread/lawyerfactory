...
class FormSelector:
    ...
    def __init__(self, forms_directory: str = "docs/Court_files"):
        ...
        self.logger.info(
            f"FormSelector initialized with {len(self.available_forms)} available forms"
        )
    ...
    def select_forms(
        ...
        optional_recommendations = self._get_optional_forms(
            classification, case_context
        )

        # Apply damage-specific logic
        damage_forms = self._get_damage_specific_forms(
            classification.damages_categories
        )
        optional_recommendations.extend(damage_forms)

        # Apply urgency-based priorities
        self._apply_urgency_priorities(
        ...
        warnings = self._generate_warnings(
            classification, primary_recommendations, optional_recommendations
        )

        # Calculate estimated completion time
        total_time = sum(
            rec.confidence * self._get_form_time(rec.form_name)
    ...
    def _get_primary_forms(
        self, classification: CaseClassification
    ...
    def _get_optional_forms(
        self, classification: CaseClassification, case_context: Dict[str, Any]
    ) -> List[FormRecommendation]:
    ...
    def _should_recommend_optional_form(
        self,
        form_key: str,
        classification: CaseClassification,
        ...
        return (
            True,
            f"Standard optional form for {classification.primary_type.value} cases",
        )

    def _get_damage_specific_forms(
        self, damage_categories: List[str]
    ) -> List[FormRecommendation]:
    ...
    def _apply_urgency_priorities(
        self,
        primary_recs: List[FormRecommendation],
        optional_recs: List[FormRecommendation],
        urgency: str,
    ):
    ...
    def _generate_warnings(
        ...
        if classification.confidence < 0.6:
            warnings.append(
                f"Low classification confidence ({classification.confidence:.1%}). "
        ...
        if not primary_recs:
            warnings.append(
                "No primary forms found for this case type. Using general complaint form."
            )

        # Complex case warning
        if len(primary_recs) + len(optional_recs) > 5:
            warnings.append(
                "Complex case requiring multiple forms. Consider legal consultation."
            )

        # Urgency warning
        if classification.urgency_level == "high":
            warnings.append(
                "High urgency case. Prioritize essential forms and consider expedited filing."
            )
    ...
    def _generate_selection_reasoning(
        ...
        if optional_recs:
            reasoning_parts.append(
                f"Recommended {len(optional_recs)} optional forms for completeness."
            )

        if classification.damages_categories:
            reasoning_parts.append(
                f"Forms selected accommodate {', '.join(classification.damages_categories)} damages."
            )
...
def test_form_selector():
    ...
    for i, classification in enumerate(test_classifications, 1):
        ...
        for form in result.primary_forms:
            print(
                f"  - {form.form_name} (Priority: {form.priority}, Confidence: {form.confidence:.1%})"
            )
