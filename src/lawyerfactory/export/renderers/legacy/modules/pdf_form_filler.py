...
class PDFFormFiller:
    ...
    def fill_form(
        self, mapping_result: MappingResult, output_filename: Optional[str] = None
    ) -> FillingResult:
        """
        ...
        for mapping in mapping_result.mappings:
            try:
                ...
                if mapping.confidence < 0.7:
                    warnings.append(
                        f"Low confidence mapping for {mapping.field_name}: {mapping.confidence:.1%}"
                    )

            except Exception as e:
                error_msg = (
                    f"Error processing mapping for {mapping.field_name}: {str(e)}"
                )
        ...
        total_fields = len(mapping_result.mappings) + len(
            mapping_result.unmapped_fields
        )
        completion_percentage = (
            (filled_count / total_fields * 100) if total_fields > 0 else 0.0
        )

        # Add warnings for unmapped fields
        if mapping_result.unmapped_fields:
            warnings.append(
                f"{len(mapping_result.unmapped_fields)} fields remain unmapped"
            )

        # Add warnings for missing data
        if mapping_result.missing_data:
            warnings.append(
                f"Missing required data: {', '.join(mapping_result.missing_data)}"
            )
    ...
    def fill_multiple_forms(
        self, mapping_results: List[MappingResult], case_prefix: str = "case"
    ) -> List[FillingResult]:
        """
        ...
        for i, mapping_result in enumerate(mapping_results):
            ...
            try:
                ...
                if result.success:
                    ...
                    self.logger.warning(
                        f"Failed to fill {mapping_result.form_name}: {result.errors}"
                    )
    ...
    def create_form_package(
        ...
        package_completion = (
            (total_filled_fields / total_fields * 100) if total_fields > 0 else 0.0
        )
        ...
        self.logger.info(
            f"  - {len(successful_forms)}/{len(mapping_results)} forms filled successfully"
        )
    ...
    def validate_filled_form(self, output_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            completion_rate = (
                (filled_fields_count / len(filled_fields) * 100) if filled_fields else 0
            )
...
def test_pdf_form_filler():
    ...
    try:
        ...
        package = filler.create_form_package(
            [mock_mapping_result], "Tesla Contract Case", "2024-001"
        )

        print("\nPackage Summary:")
        print(f"  Case: {package['case_name']}")
        print(
            f"  Forms: {package['successful_forms']}/{package['total_forms']} successful"
        )
