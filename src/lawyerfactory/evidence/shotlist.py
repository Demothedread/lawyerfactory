...
class ShotlistBuilder:
    ...
    def build(self, evidence_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not validation_report["valid"]:
                logger.error(
                    f"Evidence validation failed: {validation_report['error']}"
                )
                return {
                    "success": False,
    ...
    def _create_evidence_row(
        ...
        if isinstance(evidence_info, str):
            ...
            return {
                ...
                "timestamp": evidence_info.get(
                    "timestamp", evidence_info.get("date", "")
                ),
                "summary": evidence_info.get(
                    "summary",
                    evidence_info.get("description", evidence_info.get("content", "")),
                ),
                "entities": evidence_info.get(
                    "entities", evidence_info.get("parties", [])
                ),
                "citations": evidence_info.get(
                    "citations", evidence_info.get("references", [])
                ),
            }
    ...
    def _update_statistics(
        self, evidence_rows: List[Dict[str, Any]], validation_report: Dict[str, Any]
    ) -> None:
        """
    ...
    def validate_evidence_data(self, evidence_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(evidence_data, dict):
            ...
            validation_report["issues"].append(
                f"Evidence data must be a dictionary, got {type(evidence_data)}"
            )
        ...
        if len(evidence_data) == 0:
            ...
            validation_report["recommendations"].append(
                "Add evidence references to improve shot list quality"
            )
        ...
        if structured_count > 0:
            validation_report["recommendations"].append(
                f"Found {structured_count} structured evidence entries - good data quality"
            )

        if string_count > structured_count:
            validation_report["recommendations"].append(
                "Consider providing more structured evidence data for better shot list details"
            )
...
def build_shotlist_from_facts_matrix(
    facts_matrix: Dict[str, Any], output_directory: Optional[str] = None
) -> Dict[str, Any]:
    """
