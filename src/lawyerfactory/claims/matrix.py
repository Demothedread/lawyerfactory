...
class ComprehensiveClaimsMatrixIntegration:
    ...
    def get_comprehensive_definition(self, session_id: str) -> Optional[LegalDefinition]:
    ...
    def expand_clickable_term(self, session_id: str, term_text: str) -> Optional[ClickableTerm]:
    ...
    def get_provable_questions(self, session_id: str, element_name: str) -> List[ProvableQuestion]:
    ...
    def build_interactive_decision_tree(self, session_id: str, element_name: str) -> Dict[str, Any]:
    ...
    def enhance_with_legal_research(self, session_id: str) -> Dict[str, Any]:
    ...
    def generate_attorney_ready_analysis(self, session_id: str) -> Optional[AttorneyReadyAnalysis]:
        ...
        try:
            ...
            analysis_id = (
                f"attorney_analysis_{session_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
            ...
            discovery_recs = self._generate_discovery_recommendations(session, element_breakdowns)
            ...
            jury_instructions = self._get_california_jury_instructions(session.cause_of_action)
            ...
            # Store analysis results through unified storage pipeline
            case_id = f"claims_analysis_{session_id}"
            storage_metadata = {
                "session_metadata": {
                    "case_facts_count": len(session.case_facts),
                    "elements_analyzed": list(element_breakdowns.keys()),
                    "confidence_assessments": session.confidence_assessments,
                    "clickable_terms_expanded": list(session.clickable_terms_expanded.keys()),
                    "analysis_path_length": len(session.analysis_path),
                },
                "generation_timestamp": datetime.now().isoformat(),
                "analysis_phase": "attorney_ready",
            }

            storage_result = self.store_analysis_results(
                attorney_analysis, case_id, storage_metadata
            )

            # Add storage information to the analysis object
            if storage_result.get("success"):
                # Add ObjectIDs to the analysis for future reference
                attorney_analysis.object_ids = storage_result.get("object_ids", [])
                attorney_analysis.storage_metadata = storage_result

                logger.info(
                    f"Analysis {analysis_id} stored with {storage_result.get('stored_count', 0)} objects"
                )
            ...
            if format_type == "comprehensive":
                return {
                    "header": {
                        ...
                        "cause_of_action": analysis.cause_of_action.replace("_", " ").title(),
                        "jurisdiction": (
                ...
                return {
                    ...
                    "key_authorities": analysis.comprehensive_definition.authority_citations[:3],
    ...
    def _assess_case_strength(self, session: InteractiveAnalysisSession) -> Dict[str, Any]:
        ...
        return {
            ...
            "critical_gaps": [f"Strengthen evidence for {elem}" for elem in weak_elements],
        }
    ...
    def _generate_discovery_recommendations(
        ...
        for element, breakdown in element_breakdowns.items():
            if element == "causation":
                ...
                recommendations.append("Request medical records and financial documentation")
            elif element == "duty":
                recommendations.append("Research applicable statutes and industry standards")
    ...
    def _identify_expert_witness_needs(
        ...
        if cause_of_action == "negligence":
            if "duty" in element_breakdowns or "breach" in element_breakdowns:
                expert_needs.append("Standard of care expert for professional malpractice")
    ...
    def _identify_alternative_theories(
        ...
        if "false" in fact_text and "statement" in fact_text and primary_cause != "fraud":
    ...
    def store_analysis_results(
        self,
        analysis: AttorneyReadyAnalysis,
        case_id: str,
        metadata: Optional[Dict[str, Any]] = None,
    ) -> Dict[str, Any]:
        """
        Store attorney-ready analysis results through unified storage pipeline.

        Args:
            analysis: Complete attorney-ready analysis
        ...
        try:
            ...
            # Convert analysis to storable format
            analysis_content = self._format_analysis_for_storage(analysis)
            ...
            if storage_result.success:
                ...
                # Store individual element breakdowns as separate objects
                for element_name, breakdown in analysis.element_breakdowns.items():
                    element_content = self._format_element_breakdown_for_storage(
                        element_name, breakdown, analysis
                    )
                    ...
                    if element_storage.success:
                        object_ids.append(element_storage.object_id)
                        stored_results.append(
                            {
                                "object_id": element_storage.object_id,
                                "element_name": element_name,
                                "parent_analysis_id": analysis.analysis_id,
                            }
                        )
    ...
    def retrieve_analysis_by_object_id(self, object_id: str) -> Dict[str, Any]:
        """
        Retrieve analysis results using ObjectID.

        Args:
            object_id: The ObjectID to retrieve

        Returns:
            Analysis data from unified storage
        """
        try:
            evidence_data = self.unified_storage.get_evidence(object_id)

            if "error" in evidence_data:
                return evidence_data

            # Enhance with analysis-specific data
            metadata = evidence_data.get("metadata", {})
            analysis_data = {
                "object_id": object_id,
                "analysis_id": metadata.get("analysis_id"),
                "cause_of_action": metadata.get("cause_of_action"),
                "jurisdiction": metadata.get("jurisdiction"),
                "element_count": metadata.get("element_count"),
                "confidence_assessment": metadata.get("confidence_assessments"),
            ...
            if "s3" in evidence_data.get("available_tiers", []):
                ...
                if s3_content and isinstance(s3_content, bytes):
                    try:
                        analysis_data["content"] = s3_content.decode("utf-8")
                    except UnicodeDecodeError:
                        analysis_data["content"] = f"[Binary content: {len(s3_content)} bytes]"

            return analysis_data

        except Exception as e:
            logger.error(f"Failed to retrieve analysis by ObjectID {object_id}: {e}")
            return {"error": str(e)}

    def search_analysis_evidence(
        self, query: str, case_id: Optional[str] = None, cause_of_action: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        Search analysis evidence using unified storage.

        Args:
            query: Search query
            case_id: Optional case ID to filter results
            cause_of_action: Optional cause of action to filter results

        Returns:
            List of matching analysis evidence
        """
        try:
            search_results = self.unified_storage.search_evidence(query, search_tier="vector")

            # Filter by case_id and cause_of_action if provided
            if case_id:
                search_results = [
                    result
            ...
            if cause_of_action:
                search_results = [
                    result
                    for result in search_results
                    if result.get("metadata", {}).get("cause_of_action") == cause_of_action
                ]

            # Enhance results with analysis-specific data
            enhanced_results = []
            for result in search_results:
            ...
            logger.error(f"Failed to search analysis evidence: {e}")
            return []

    def _format_analysis_for_storage(self, analysis: AttorneyReadyAnalysis) -> str:
        """
        Format attorney-ready analysis for storage as text content.
        ...
        """
        content = f"""
ATTORNEY-READY LEGAL ANALYSIS
============================

Analysis ID: {analysis.analysis_id}
Session ID: {analysis.session_id}
Cause of Action: {analysis.cause_of_action.replace('_', ' ').title()}
Jurisdiction: {analysis.jurisdiction}
Created: {analysis.created_at.strftime('%Y-%m-%d %H:%M:%S')}

LEGAL DEFINITION
================
{analysis.comprehensive_definition.primary_definition}

Authority Citations:
{chr(10).join(f"• {citation}" for citation in analysis.comprehensive_definition.authority_citations)}

CASE STRENGTH ASSESSMENT
========================
Overall Strength: {analysis.case_strength_assessment.get('overall_strength', 'Unknown')}
Confidence Score: {analysis.case_strength_assessment.get('confidence_score', 0.0):.2f}

Strong Elements:
{chr(10).join(f"• {element}" for element in analysis.case_strength_assessment.get('strong_elements', []))}

Weak Elements:
{chr(10).join(f"• {element}" for element in analysis.case_strength_assessment.get('weak_elements', []))}
...
"""
        ...
        for element_name, breakdown in analysis.element_breakdowns.items():
            ...
            content += f"Definition: {breakdown.element_definition}\n"
            content += f"Essential: {'Yes' if breakdown.is_essential else 'No'}\n"
            if breakdown.provable_questions:
                content += "Provable Questions:\n"
                for question in breakdown.provable_questions[:3]:  # Limit to first 3
...
{chr(10).join(f"• {rec}" for rec in analysis.discovery_recommendations[:5])}

Expert Witness Needs:
{chr(10).join(f"• {expert}" for expert in analysis.expert_witness_needs)}

Alternative Theories:
{chr(10).join(f"• {theory}" for theory in analysis.alternative_theories[:3])}

Practice Guidance:
{chr(10).join(f"• {guidance}" for guidance in analysis.practice_guidance[:5])}

CALIFORNIA AUTHORITIES
======================
{chr(10).join(f"• {auth.get('authority_name', 'Unknown')}" for auth in analysis.california_authorities[:5])}

JURY INSTRUCTIONS
================
{chr(10).join(f"• {instruction}" for instruction in analysis.jury_instructions[:5])}

CASE LAW PRECEDENTS
===================
{chr(10).join(f"• {precedent.get('citation', 'Unknown')}: {precedent.get('principle', '')}" for precedent in analysis.case_law_precedents[:3])}
"""

        return content.strip()

    def _format_element_breakdown_for_storage(
        self, element_name: str, breakdown: ElementBreakdown, analysis: AttorneyReadyAnalysis
    ) -> str:
        """
        Format individual element breakdown for storage.

        Args:
            element_name: Name of the legal element
            breakdown: Element breakdown details
            analysis: Parent analysis for context

        Returns:
            Formatted text content for element
        """
        content = f"""
ELEMENT BREAKDOWN: {element_name.upper()}
=====================================

Parent Analysis: {analysis.analysis_id}
Cause of Action: {analysis.cause_of_action.replace('_', ' ').title()}
Jurisdiction: {analysis.jurisdiction}

ELEMENT DEFINITION
==================
{breakdown.element_definition}

Essential Element: {'Yes' if breakdown.is_essential else 'No'}
Element Type: {breakdown.element_type}
...
"""
        ...
        if breakdown.provable_questions:
            for i, question in enumerate(breakdown.provable_questions, 1):
                content += f"{i}. {question.question_text}\n"
                if question.follow_up_questions:
                    for follow_up in question.follow_up_questions[:2]:  # Limit follow-ups
                        content += f"   - {follow_up}\n"
...
=====================
{chr(10).join(f"• {req}" for req in breakdown.evidence_requirements)}

DECISION TREE ANALYSIS
======================
{chr(10).join(f"• {outcome}" for outcome in breakdown.decision_tree_analysis)}
"""
...
if __name__ == "__main__":
    ...
    try:
        ...
        session_id = integration.start_interactive_analysis("ca_state", "negligence", test_facts)
        ...
        if attorney_analysis:
            report = integration.export_analysis_report(attorney_analysis, "comprehensive")
            print(f"\nExported comprehensive report with {len(report)} sections")

            summary_report = integration.export_analysis_report(attorney_analysis, "summary")
            print(f"Exported summary report: {summary_report.get('case_overview', 'N/A')}")
