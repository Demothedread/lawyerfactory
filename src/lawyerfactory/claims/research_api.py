...
class ClaimsMatrixResearchAPI:
    ...
    async def get_research_result(self, request_id: str) -> ClaimsMatrixResearchResponse:
        ...
        try:
            ...
            base_results = await self.research_integration.execute_research_request(
                research_request
            )
    ...
    async def _validate_research_request(
        self, request: ClaimsMatrixResearchRequest
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self.jurisdiction_manager.select_jurisdiction(request.jurisdiction):
                return {
                    "valid": False,
                    "error": f"Invalid jurisdiction: {request.jurisdiction}",
                }
            ...
            if not cause_validation["valid"]:
                return {
                    "valid": False,
                    "error": cause_validation.get("reason", "Invalid cause of action"),
                }
    ...
    async def _check_comprehensive_cache(
        self, request: ClaimsMatrixResearchRequest
    ) -> Optional[ClaimsMatrixResearchResponse]:
        """Check for cached comprehensive results"""
        try:
            cache_key = self._generate_comprehensive_cache_key(request)
            cached_data = await self.cache_manager.get_cached_research(
                cache_key, request.jurisdiction
            )
    ...
    async def _process_research_citations(
        self,
        citations: List[Any],
        response: ClaimsMatrixResearchResponse,
    ...
    async def _validate_research_authorities(
        self, citations: List[Any], request: ClaimsMatrixResearchRequest
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            bluebook_validations = await self.authority_validator.validate_bluebook_citations(
                citations
            )
            ...
            return {
                ...
                "bluebook_compliance": {
                    ...
                    "compliance_rate": (
                        len([v for v in bluebook_validations if v.is_compliant]) / len(citations)
                        if citations
                        else 0
                    ),
    ...
    async def _get_legal_definitions(
        self, request: ClaimsMatrixResearchRequest
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for term in legal_terms:
                cached_def = await self.cache_manager.get_cached_definition(
                    term, request.jurisdiction
                )
    ...
    async def _cache_comprehensive_result(
        self,
        request: ClaimsMatrixResearchRequest,
        response: ClaimsMatrixResearchResponse,
    ):
    ...
    async def _integrate_with_claims_matrix(
        self,
        request: ClaimsMatrixResearchRequest,
        response: ClaimsMatrixResearchResponse,
    ):
        ...
        try:
            ...
            logger.debug(
                f"Integrated research results with Claims Matrix for request {request.request_id}"
            )
    ...
    async def trigger_cause_research(
        self,
    ...
    async def invalidate_research_cache(
        self, jurisdiction: str = None, cause_of_action: str = None
    ) -> Dict[str, Any]:
...
async def create_claims_matrix_research_api(
    enhanced_kg: EnhancedKnowledgeGraph,
    courtlistener_token: str = None,
    scholar_contact_email: str = None,
) -> ClaimsMatrixResearchAPI:
