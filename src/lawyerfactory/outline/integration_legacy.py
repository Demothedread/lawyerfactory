...
class SkeletalOutlineIntegration:
    ...
    def __init__(
        ...
        self.prompt_orchestrator = PromptChainOrchestrator(enhanced_kg, llm_service)
    ...
    async def start_skeletal_outline_workflow(
        self, case_id: str, session_id: str, document_type: str = "complaint"
    ) -> str:
        ...
        try:
            ...
            logger.info(
                f"Starting skeletal outline workflow {workflow_id} for case {case_id}"
            )
    ...
    async def _execute_workflow(self, workflow: SkeletalOutlineWorkflow):
        ...
        try:
            ...
            prompt_result = await self.prompt_orchestrator.execute_prompt_chain(
                skeletal_outline
            )
    ...
    async def _save_workflow_results(self, workflow: SkeletalOutlineWorkflow):
        ...
        try:
            ...
            if (
                workflow.prompt_chain_result
                and workflow.prompt_chain_result.final_document
            ):
                document_file = (
                    self.storage_path / f"{workflow.workflow_id}_complaint.txt"
                )
        ...
        if workflow.prompt_chain_result:
            status_data["generation_results"] = {
                ...
                "sections_generated": len(
                    workflow.prompt_chain_result.generated_sections
                ),
    ...
    async def regenerate_section(
        self, workflow_id: str, section_id: str, custom_prompt: Optional[str] = None
    ) -> bool:
        ...
        try:
            ...
            if workflow.prompt_chain_result:
                ...
                # Update results
                workflow.prompt_chain_result.generated_sections[section_id] = (
                    new_content
                )
                workflow.prompt_chain_result.final_document = (
                    self.prompt_orchestrator._compile_final_document(
...
class SkeletalOutlineAPI:
        ...
        try:
            ...
            if not case_id or not session_id:
                return web.json_response(
                    {"error": "case_id and session_id are required"}, status=400
                )
    ...
    async def get_generated_document(self, request) -> web.Response:
        ...
        try:
            ...
            if not document:
                return web.json_response(
                    {"error": "Document not found or not ready"}, status=404
                )
    ...
    async def get_section_content(self, request) -> web.Response:
        ...
        try:
            ...
            if not content:
                return web.json_response({"error": "Section not found"}, status=404)
    ...
    async def list_workflows(self, request) -> web.Response:
        ...
        try:
            ...
            return web.json_response(
                {"workflows": workflows, "total_count": len(workflows)}
            )
    ...
    async def regenerate_section(self, request) -> web.Response:
        ...
        try:
            ...
            if success:
                ...
                return web.json_response(
                    {"error": "Failed to regenerate section"}, status=400
                )
...
def setup_skeletal_outline_routes(app, api_instance: SkeletalOutlineAPI):
    ...
    app.router.add_get(
        "/api/skeletal-outline/workflow/{workflow_id}/status",
        api_instance.get_workflow_status,
    )

    # Document retrieval routes
    app.router.add_get(
        "/api/skeletal-outline/workflow/{workflow_id}/document",
        api_instance.get_generated_document,
    )
    app.router.add_get(
        "/api/skeletal-outline/workflow/{workflow_id}/section/{section_id}",
        api_instance.get_section_content,
    )

    # Section management routes
    app.router.add_post(
        "/api/skeletal-outline/workflow/{workflow_id}/section/{section_id}/regenerate",
        api_instance.regenerate_section,
    )

    logger.info("Skeletal outline API routes configured")


def create_skeletal_outline_system(
    enhanced_kg: EnhancedKnowledgeGraph,
    claims_matrix: ComprehensiveClaimsMatrixIntegration,
    evidence_api: EvidenceAPI,
    llm_service=None,
