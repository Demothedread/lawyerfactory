...
class LegalProcedureBot(Bot, AgentInterface):
    ...
    async def process(self, message: str) -> str:
        ...
        try:
            ...
            if issues:
                return (
                    f"Procedural review completed for: '{message}'\nIssues to address:\n"
        ...
        for issue in issues:
            if "jurisdiction" in issue.lower():
                recommendations.append(
                    "Add proper federal/state jurisdiction basis per 28 U.S.C. § 1331 or § 1332"
                )
            elif "venue" in issue.lower():
                recommendations.append("Include venue statement per 28 U.S.C. § 1391")
            elif "prayer" in issue.lower():
                recommendations.append(
                    "Add prayer for relief section requesting specific damages and relief"
                )
            elif "signature" in issue.lower():
                recommendations.append(
                    "Add proper signature block with attorney information per Fed. R. Civ. P. 11"
                )
            else:
