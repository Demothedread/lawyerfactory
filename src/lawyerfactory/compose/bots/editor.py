...
class LegalEditorBot(Bot, AgentInterface):
        ...
        try:
            ...
            if issues:
                return flag_for_review(
                    f"Professional review feedback for: '{message}'", issues
                )
            else:
    ...
    async def execute_task(
        ...
        try:
            ...
            if review_type == "compliance" or review_type == "rule_12b6":
                result = await self._professional_compliance_review(
                    content, causes_of_action, research_findings, context
                )
            elif review_type == "citation_review":
                result = await self._professional_citation_review(
                    content, research_findings, context
                )
            elif review_type == "legal_brief":
                result = await self._professional_legal_brief_review(
                    content, causes_of_action, research_findings, context
                )
            ...
            return {
                ...
                "attorney_review_required": result.get(
    ...
    async def _professional_compliance_review(
        self,
        content: str,
        causes_of_action: List[Dict],
        ...
        try:
            ...
            reviewed_content = (
                flag_for_review(content, all_issues) if all_issues else content
            )
            ...
            if not all_issues:
                feedback_sections.append(
                    "**COMPLIANCE STATUS:** Document meets Rule 12(b)(6) requirements and citation standards."
                )

            feedback_sections.append(
                f"**REVIEW SUMMARY:** {len(compliance_issues)} compliance issues, {len(citation_issues)} citation issues found."
            )
    ...
    async def _professional_citation_review(
        self, content: str, research_findings: Dict, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Focused citation review using validation modules"""
        try:
            citation_issues = (
                validate_citations(research_findings) if research_findings else []
            )
            ...
            if citation_issues:
                feedback = "**CITATION REVIEW:**\n" + "\n".join(
                    f"• {issue}" for issue in citation_issues
                )
    ...
    async def _professional_legal_brief_review(
        self,
        content: str,
        causes_of_action: List[Dict],
        research_findings: Dict,
        ...
        compliance_result = await self._professional_compliance_review(
            content, causes_of_action, research_findings, context
        )
        ...
        if (
            "LEGAL STANDARD" not in content.upper()
            and "STANDARD OF REVIEW" not in content.upper()
        ):
        ...
        # Combine all issues
        all_issues = (
            compliance_result.get("compliance_issues", [])
    ...
    async def _professional_format_review(
        ...
        if not any(
            court_indicator in content.upper()
            for court_indicator in [
    ...
    async def _comprehensive_professional_review(
        self,
        content: str,
        causes_of_action: List[Dict],
        research_findings: Dict,
        ...
        format_result = await self._professional_format_review(
            compliance_result["content"], context
        )
...
"""
        ...
        return {
            ...
            "changes_made": len(all_issues),
            "ready_for_filing": len(
                compliance_result.get("compliance_issues", [])
                + compliance_result.get("citation_issues", [])
            )
            == 0,
            "attorney_review_required": len(
                compliance_result.get("compliance_issues", [])
                + compliance_result.get("citation_issues", [])
            )
            > 0,
    ...
    def _check_document_structure(self, content: str) -> List[str]:
        ...
        if not any(
            ...
            for section in ["PLAINTIFF", "DEFENDANT", "PETITIONER", "RESPONDENT"]
        ):
        ...
        if not any(
            damages in content.upper() for damages in ["DAMAGES", "RELIEF", "PRAYER"]
        ):
