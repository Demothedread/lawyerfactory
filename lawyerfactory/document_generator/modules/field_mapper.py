...
from .pdf_handler import PDFHandler, PDFFormField
...
class FieldMapper:
    ...
    def _initialize_mapping_patterns(self):
        ...
        self.field_priority = [
            'plaintiff', 'defendant', 'case_number', 'court_name',
            'incident_date', 'contract_date', 'filing_date',
            'damages', 'medical_expenses', 'facts',
            'plaintiff_address', 'defendant_address', 'plaintiff_phone'
        ]
    
    def map_fields(self, form_name: str, case_data: Dict[str, Any], 
                   classification: CaseClassification) -> MappingResult:
        """
        ...
        missing_data = self._identify_missing_data(enhanced_case_data, classification.primary_type)
        ...
        # Generate warnings
        warnings = self._generate_mapping_warnings(mappings, unmapped_fields, missing_data)
    ...
    def _enhance_case_data(self, case_data: Dict[str, Any], 
                          classification: CaseClassification) -> Dict[str, Any]:
        ...
        if 'facts_narrative' not in enhanced and 'facts' in enhanced:
            if isinstance(enhanced['facts'], list):
                facts_texts = [fact.get('text', '') for fact in enhanced['facts'] if isinstance(fact, dict)]
    ...
    def _map_single_field(self, field: PDFFormField, case_data: Dict[str, Any]) -> Optional[FieldMapping]:
            ...
            for pattern in pattern_info['patterns']:
                if re.search(pattern, field_name_lower, re.IGNORECASE):
                    ...
                    if data_key in case_data:
                        ...
                        if transform_func:
                            transformed_value, transform_desc = self._apply_transformation(
    ...
    def _apply_transformation(self, value: Any, transform_name: str, 
                            field_type: str) -> Tuple[str, Optional[str]]:
        ...
        if transform_name == 'name_format':
            ...
            if len(phone_str) == 10:
                return f"({phone_str[:3]}) {phone_str[3:6]}-{phone_str[6:]}", "phone number formatting"
            return str(value), "string conversion"
    ...
    def _calculate_mapping_confidence(self, field_name: str, pattern: str, 
        ...
        priority_boost = {
            'plaintiff': 0.2, 'defendant': 0.2, 'case_number': 0.15,
            'court_name': 0.1, 'incident_date': 0.1
        }
    ...
    def _handle_generic_field_names(self, field: PDFFormField,
                                   case_data: Dict[str, Any]) -> Optional[FieldMapping]:
    ...
    def _fuzzy_field_match(self, field: PDFFormField,
                          case_data: Dict[str, Any]) -> Optional[FieldMapping]:
        ...
        for data_key, data_value in case_data.items():
            ...
            if (data_key_lower in field_name_lower or
                field_name_lower in data_key_lower):
    ...
    def _identify_missing_data(self, case_data: Dict[str, Any], 
                              case_type: CaseType) -> List[str]:
        """Identify missing data that should be collected for the case type."""
        
        required_fields = {
            CaseType.BREACH_OF_CONTRACT: [
                'plaintiff_name', 'defendant_name', 'contract_date', 'damages_amount'
            ],
            CaseType.PERSONAL_INJURY: [
                'plaintiff_name', 'defendant_name', 'incident_date', 'medical_expenses'
            ],
            CaseType.NEGLIGENCE: [
                'plaintiff_name', 'defendant_name', 'incident_date'
            ],
            CaseType.PRODUCT_LIABILITY: [
                'plaintiff_name', 'defendant_name', 'incident_date', 'product_description'
            ]
        }
    ...
    def _generate_mapping_warnings(self, mappings: List[FieldMapping], 
                                  unmapped_fields: List[str], 
                                  missing_data: List[str]) -> List[str]:
        ...
        # No mappings at all
        if not mappings:
            warnings.append("No form fields could be mapped - check case data completeness")
    ...
    def get_mapping_summary(self, result: MappingResult) -> Dict[str, Any]:
        ...
        return {
            ...
            'high_confidence_mappings': len([m for m in result.mappings if m.confidence >= 0.8]),
            'medium_confidence_mappings': len([m for m in result.mappings if 0.6 <= m.confidence < 0.8]),
            'low_confidence_mappings': len([m for m in result.mappings if m.confidence < 0.6]),
            ...
            'ready_for_filling': result.mapping_confidence > 0.7 and len(result.missing_data) == 0
        }
...
def test_field_mapper():
    ...
    if result.mappings:
        ...
        for mapping in result.mappings[:5]:  # Show first 5 mappings
            ...
            print(f"    Confidence: {mapping.confidence:.1%}, Source: {mapping.source_data}")
