...
from ..util import (check_spacy_env_vars, download_file, ensure_pathy,
                    get_checksum, get_hash, make_tempdir, upload_file)
...
class RemoteStorage:
    ...
    def push(self, path: Path, command_hash: str, content_hash: str) -> "CloudPath":
        ...
        loc = self.root / path
        if not loc.exists():
            raise OSError(f"Cannot push {loc}: does not exist.")
