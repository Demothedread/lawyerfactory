...
def color(
    ...
    if underline:
        styles.append("4")
    if fg:
        styles.append(f"38;5;{fg}")
    if bg:
        styles.append(f"48;5;{bg}")
...
def format_repr(obj: Any, max_len: int = 50, ellipsis: str = "...") -> str:
    ...
    if len(string) >= max_len:
        half = int(max_len / 2)
        return f"{string[:half]} {ellipsis} {string[-half:]}"
    else:
        return string
...
def diff_strings(
    ...
    b: str,
    fg: str | int = "black",
    bg: tuple[str, str] | tuple[int, int] = ("green", "red"),
...
def get_raw_input(
    description: str, default: str | bool | None = False, indent: int = 4
) -> str:
    ...
    """
    additional = f" (default: {default})" if default else ""
    prompt = wrap(f"{description}{additional}: ", indent=indent)
