...
class Printer:
    def __init__(
        ...
        env_no_pretty = os.getenv(f"{env_prefix}_NO_PRETTY", False)
    ...
    def fail(
        ...
        show: bool = True,
        spaced: bool = False,
        exits: Literal[0, False] | None = None,
    ):
        ...
    ...
    def text(
        ...
        if self.pretty:
            ...
            if icon:
                title = locale_escape(f"{icon} {title}").strip()
            if self.show_color:
    ...
    def divider(
        ...
        if self.pretty:
            icon = self.icons.get(cast(str, icon))
            if icon:
                text = locale_escape(f"{icon} {text}").strip()
    ...
    def table(self, data: Collection | dict, **kwargs):
