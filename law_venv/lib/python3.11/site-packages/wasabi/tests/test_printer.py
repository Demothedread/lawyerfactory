...
def test_printer():
    ...
    if SUPPORTS_ANSI and not NO_UTF8:
        assert good == f"\x1b[38;5;2m\u2714 {text}\x1b[0m"
        assert fail == f"\x1b[38;5;1m\u2718 {text}\x1b[0m"
        assert warn == f"\x1b[38;5;3m\u26a0 {text}\x1b[0m"
        assert info == f"\x1b[38;5;4m\u2139 {text}\x1b[0m"
    if SUPPORTS_ANSI and NO_UTF8:
        assert good == f"\x1b[38;5;2m[+] {text}\x1b[0m"
        assert fail == f"\x1b[38;5;1m[x] {text}\x1b[0m"
        assert warn == f"\x1b[38;5;3m[!] {text}\x1b[0m"
        assert info == f"\x1b[38;5;4m[i] {text}\x1b[0m"
    if not SUPPORTS_ANSI and not NO_UTF8:
        assert good == f"\u2714 {text}"
        assert fail == f"\u2718 {text}"
        assert warn == f"\u26a0 {text}"
        assert info == f"\u2139 {text}"
    if not SUPPORTS_ANSI and NO_UTF8:
        assert good == f"[+] {text}"
        assert fail == f"[x] {text}"
        assert warn == f"[!] {text}"
        assert info == f"[i] {text}"
...
def test_printer_custom():
    ...
    warning = p.warn(text)
    if SUPPORTS_ANSI and not NO_UTF8:
        assert purple_question == f"\x1b[38;5;99m? {text}\x1b[0m"
        assert warning == f"\x1b[38;5;3m\u26a0\ufe0f {text}\x1b[0m"
    if SUPPORTS_ANSI and NO_UTF8:
        assert purple_question == f"\x1b[38;5;99m? {text}\x1b[0m"
        assert warning == f"\x1b[38;5;3m?? {text}\x1b[0m"
    if not SUPPORTS_ANSI and not NO_UTF8:
        assert purple_question == f"? {text}"
        assert warning == f"\u26a0\ufe0f {text}"
    if not SUPPORTS_ANSI and NO_UTF8:
        assert purple_question == f"? {text}"
        assert warning == f"?? {text}"
...
def test_printer_spaced():
    ...
    text = "This is a test."
    assert p.good(text) == text
    assert p.good(text, spaced=True) == f"\n{text}\n"
