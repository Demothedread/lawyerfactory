    ...
    <NeonGradientCard className={`neon-phase-card ${isActive ? 'active' : ''} ${phaseState?.status || 'idle'} phase-${phaseState?.status}`}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
          <Box sx={{ flex: 1 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
              ...
              {/* Enhanced: Use SparklesText for active phase names */}
              {isActive ? (
                <SparklesText text={phase.name} className="phase-title-sparkles" />
              ) : (
                <Typography
                  variant="h6"
                  className="neon-text"
      ...
      {phaseState?.outputs && phaseState.outputs.length > 0 && (
        <Box sx={{ mb: 2 }}>
          ...
          >
            ✅ OUTPUTS ({phaseState.outputs.length})
          </Typography>
      ...
      {phase?.id === 'phaseA03_outline' && phaseState?.status === 'completed' && !loadingDeliverables && (
        <Box sx={{ mb: 2 }}>
          ...
          {/* Review All Deliverables Button */}
      ...
      {phaseState?.errors && phaseState.errors.length > 0 && (
        ...
        >
          ...
          >
            ⚠️ {phaseState.errors[phaseState.errors.length - 1].error}
          </Typography>
        </Box>
      )}
